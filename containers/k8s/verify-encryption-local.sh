#!/bin/bash
# verify-encryption-local.sh — On-CP-image etcd at-rest encryption
# verifier. Copied into /usr/libexec/verify-encryption.sh by
# containers/k8s/Containerfile.
#
# Originally extracted from the local-mode branch of the (now-deleted)
# scripts/verify-encryption.sh (v0.1.0 cutover, #353). The client-side
# bash twin was replaced by `hbird verify encryption` (Rust); this
# file is the on-image counterpart that `hbird verify encryption`
# ssh-execs on the CP via the `cp_ssh_lenient` shim.
#
# Strategy on the CP:
...
# ──────────────────────────────────────────────────────────────────────
# Local mode (on the CP). Run kubectl/crictl/etcdctl directly.
# ──────────────────────────────────────────────────────────────────────
...
if ! command -v kubectl >/dev/null 2>&1; then
  log "kubectl not on PATH (this script runs on the CP — `hbird verify encryption` ssh-execs it via /usr/libexec)"
  exit 2
fi

if ! kubectl get --raw=/healthz >/dev/null 2>&1; then
  log "kubectl can't reach the API server (KUBECONFIG=$KUBECONFIG)"
  log "  hint: this script is meant to run on the CP via `hbird verify encryption`'s ssh exec; CP must be healthy"
  exit 2
fi
