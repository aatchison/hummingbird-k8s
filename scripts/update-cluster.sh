...
run_ssh() {
  # run_ssh <user@host> <remote-cmd...>
  local target="$1"; shift
  if (( DRY_RUN == 1 )); then
    log "DRY-RUN ssh ${target} -- $*"
    return 0
  fi
  # SC2029 (client-side expansion of $*) is intentional: callers pass a
  # single quoted remote-command string they already composed locally.
...
update_cp() {
  ...
  log "ssh root@${CP_IP} bootc upgrade --apply  (auto-reboots)"
  # bootc upgrade --apply returns non-zero on "no update available". Treat
  # that as success (idempotent). Anything else is a real failure.
  if (( DRY_RUN == 0 )); then
    set +e
    ssh "${SSH_OPTS[@]}" "root@${CP_IP}" "bootc upgrade --apply"
    rc=$?
    set -e
    # The reboot side of --apply tears down the ssh connection; ssh exits
    # 255 in that case. Treat 0 and 255 as expected.
    case "$rc" in
      0|255) ;;
      *) fail "bootc upgrade --apply on CP exited with rc=${rc}" ;;
    esac
  else
    log "DRY-RUN ssh root@${CP_IP} bootc upgrade --apply"
  fi
...
update_worker() {
  ...
  log "ssh root@${ip} bootc upgrade --apply  (auto-reboots)"
  if (( DRY_RUN == 0 )); then
    set +e
    ...
    case "$rc" in
      0|255) ;;
      *) fail "bootc upgrade --apply on ${name} exited with rc=${rc}" ;;
    esac
  else
    log "DRY-RUN ssh root@${ip} bootc upgrade --apply"
  fi
