...
| `hbird deploy-cluster` / `hbird destroy-cluster` (Rust) | Cluster deploy / teardown. The bash twins were **removed** in the v0.2.0 cutover (#289 S4) after live validation on a KVM host; the Makefile delegates to `hbird`. Driven from `cluster.local.conf`. |
...
- **Rust rewrite shipped (epic #279); v0.1.0 partial cutover landed in #353.** Every operator-facing Makefile target has an `hbird` counterpart in `rust/crates/hbird-cli/`. Foundation crates + Phase 1A/1B/2/3/4 + tracing all landed (PRs #321 #325 #326 #330 #334 #337 #344 #346 #347). v0.1.0 cutover (#353) **removed** the bash twins for update-cluster / verify-* / export-argocd / kubectl-k8s; the Makefile recipes now delegate to `hbird` directly (cross-runtime dependency: `hbird` must be on PATH). v0.2.0 (#289 S4) flipped `make deploy-cluster` / `make destroy-cluster` to `hbird` after live validation on a KVM host and **deleted** `scripts/{deploy,destroy}-cluster.sh` + `scripts/lib/cloud-init-seed.sh`. `scripts/spawn-workers.sh` and `scripts/lib/ssh-wrap.sh` are **retained**: ssh-wrap is still sourced by `clean-vms.sh` and `switch-to-ghcr.sh`, which remain bash. See [`docs/rust-cli-migration.md`](docs/rust-cli-migration.md) for the operator-facing `make → hbird` lookup table and [`docs/rust-cli.md`](docs/rust-cli.md) for the per-phase tracking table.
