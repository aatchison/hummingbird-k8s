...
2. Check the new base's **Fedora era**: `docker run --rm quay.io/hummingbird-community/bootc-os@<new-digest> rpm -E %fedora`. If it moved past the current `ARG FEDORA_RELEASE` in the Containerfiles, bump that ARG in both flavors **in the same PR**, vendor the new era's GPG key as `containers/shared/gpg/RPM-GPG-KEY-fedora-<NN>-primary`, and re-run the resolve probes from #398. A build-time assertion fails `containerfile-build` loudly on era mismatch, so a bump PR that skips this step cannot merge green (#397, #399).
