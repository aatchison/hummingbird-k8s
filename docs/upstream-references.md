...
| Cilium project | <https://github.com/cilium/cilium> | CNI installed at first boot by `k8s-init.sh` via `cilium-cli`. Pinned at **1.16.5** in `containers/k8s/k8s-init.sh:203`, **independently of `K8S_VERSION`**. | Bumping Cilium; debugging CNI / Hubble; understanding why a K8s version bump doesn't automatically bump Cilium. | Independent-of-K8S_VERSION pin ([#303](https://github.com/aatchison/hummingbird-k8s/issues/303)); Hubble relay + metrics-server land on podman-CNI IPs first boot ([#259](https://github.com/aatchison/hummingbird-k8s/issues/259)). |
