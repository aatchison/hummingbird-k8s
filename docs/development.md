...
| `resolve_vm_ip <vm> [attempts] [interval]` | Echo first IPv4 from `virsh -c qemu:///system domifaddr <vm>`. Defaults to 1 attempt, 0s interval — pass larger values for the DHCP-not-yet-ready boot-wait case. Probes virsh up front; warns when the VM exposes multiple IPv4 addresses (first is returned). Requires local libvirt — use `resolve_cp_ip` from operator-facing scripts that may run on a workstation. |
| `resolve_cp_ip <vm> [<override-ip>]` | Workstation-aware sibling of `resolve_vm_ip`. Tries (1) explicit `<override-ip>` arg, (2) `ssh "$KVM_HOST" virsh domifaddr` when `KVM_HOST` is set (non-sudo first, then `sudo -n` fallback), (3) local `virsh` via `resolve_vm_ip`, (4) fails with a diagnostic that names both `CP_IP` and `KVM_HOST` as recovery levers. Use this from any script that may be invoked from an operator's laptop without libvirt installed (`export-argocd`, `get-kubeconfig`, `verify-hardening` since #271 F4). Scripts that re-exec via `scripts/lib/ssh-wrap.sh` keep using `resolve_vm_ip` directly — by the time IP resolution runs, they're already on the KVM host. |
