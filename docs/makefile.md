...
| `HBIRD_REMOTE_FRESHNESS_CHECK` | `1` (on)             | When set to a boolish OFF value (`0`/`false`/`no`/`off`), the C3 SSH-wrap shim skips the pre-re-exec freshness probe that compares the local repo's HEAD SHA to the remote checkout's HEAD on `$KVM_HOST`. The check is **client-side only** — never forwarded to the remote. Operator opt-out for release-train rollback drills, bisect-across-nodes, or any case where running a known-older remote checkout is intentional. See `scripts/lib/ssh-wrap.sh` (#365). |
| `HBIRD_REMOTE_STRICT`          | `0` (warn-only)      | When set to a boolish ON value (`1`/`true`/`yes`/`on`), the C3 SSH-wrap freshness check upgrades stale-remote WARNs into a hard fail (exit 1, no re-exec). Suggested for CI workflows that drive the shim from a non-KVM-host runner (current CI runs ON the KVM host so the shim never fires there). Does NOT hard-fail on network-unreachable — that's transient. **Client-side only.** (#365) |
| `HBIRD_REMOTE_LAG_THRESHOLD`   | `5`                  | Maximum number of commits the remote checkout may be BEHIND `main` before the C3 SSH-wrap freshness check emits a WARN. Default `5` swallows the common release-train rebase noise. Set to `0` to WARN on any lag. Non-numeric values fall back to the default. **Client-side only.** (#365) |
