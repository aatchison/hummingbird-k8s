...
| Phase 1B — `update-cluster` (live-execution slice; cycle 1 = `cp_kubectl` + drain/uncordon; cycle 2 = bootID gate; cycle 3 = DaemonSet-ready gate; cycle 4 = apiserver-back gate) | [#322](https://github.com/aatchison/hummingbird-k8s/issues/322) | cycles 1 (PR #325) + 2 (PR #344) + 3 (PR #346) + 4 (#329 / PR #347) landed — Phase 1B complete for cluster-rotation paths; `timer_stop`/`timer_start` (block #4) deferred outside scope (no scheduled-update timer on geary) |
